...
// ———————————————————— ADMIN DM POLLING ———————————————————— //
// MQTT doesn't reliably deliver DM messages, so we poll manually every 15s.
// We wait up to 60s for the bot to finish login (fcaApi to become available).
(function startAdminDMPolling() {
        const MAX_WAIT = 60000;
        const CHECK_INTERVAL = 2000;
        let waited = 0;

        const waitForApi = setInterval(async () => {
                waited += CHECK_INTERVAL;
                const api = global.BlackBot && global.BlackBot.fcaApi;
                if (!api) {
                        if (waited >= MAX_WAIT) {
                                clearInterval(waitForApi);
                                log.warn("DM POLL", "فشل تشغيل polling — API غير متاح بعد 60s");
                        }
                        return;
                }
                clearInterval(waitForApi);
                ...
                const lastSeen = {};
                let botID = String(global.BlackBot.botID || global.botID || "");

                const pollDMs = async () => {
                        if (!botID) botID = String(global.BlackBot.botID || global.botID || "");
                        const currentAPI = global.BlackBot.fcaApi;
                        if (!currentAPI) return;

                        for (const adminID of adminIDs) {
                                try {
                                        const history = await currentAPI.getThreadHistory(adminID, 5, null);
                                        if (!history || !history.length) continue;

                                        const latest = history
                                                .filter(m => m.body && m.body.trim() && String(m.senderID) !== botID)
                                                .pop();
                                        ...
                                        log.info("DM POLL", `📨 رسالة خاص من أدمن ${adminID}: ${body.slice(0, 60)}`);
                                        ...
                                        try {
                                                ...
                                                log.err("DM POLL", "خطأ في توجيه الرسالة للـ AI", err);
                                        }
                                } catch (_e) {}
                        }
                };
                ...
                log.info("DM POLL", `🔄 بدأ فحص رسائل الخاص كل 15 ثانية | ${adminIDs.length} أدمن`);
        }, CHECK_INTERVAL);
})();
